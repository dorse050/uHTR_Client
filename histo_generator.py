...
def generate_histos(crates, slots, n_orbits=5000, sepCapID=0, file_out_base="", new_dir="histotests"):
	...
	### check to see if new_dir exists and set it up if it does
	if not os.path.exists(new_dir):
		os.makedirs(new_dir)

	dir_path="{0}/{1}".format(cwd,new_dir)
	os.chdir(dir_path)
	...
	for crate in crates:
		for slot in slots:
			...
			p.start()
			print "slot {0} test complete".format(slot)
		print "crate {0} test complete".format(crate)
	...
	### return the the full path to new_dir
	return dir_path

if __name__=="__main__": 
	slots=[1, 2, 3, 4, 5]
	histo_tests(41, slots)
